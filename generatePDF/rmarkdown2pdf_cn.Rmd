...
table_index <- c('检测指标名称', '结果', '单位', '', '参考范围',
                 '检测指标名称', '结果', '单位', '', '参考范围')
select_column1Data <- select_column1Data %>%
  select(name, result, unit, mark, "参考范围")
...
kable(total_table, format = "latex", longtable = TRUE, booktabs = TRUE, linesep = "") %>%
  kable_styling(bootstrap_options = "bordered", full_width = T, font_size = 7.5) %>%
    row_spec(0, bold = T) %>%
    column_spec(c(1,6), width = "3.5cm") %>%
    column_spec(c(5,10), width = "1.2cm") %>%
    column_spec(c(4,9), width = "0.1cm") %>%
    column_spec(c(2,7), width = "1.2cm") %>%
    column_spec(c(3,8), width = "0.6cm")
...
\textbf{检测人：} & \textbf{审核人：} & \textbf{报告日期：} \\
\end{tabu}
